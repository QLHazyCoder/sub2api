...
// openAIStreamTerminalShouldFailover treats an empty successful terminal as a
// failed attempt only before any client output or usage was committed.
...
func openAIStreamFailedEventRetryableOnSameAccount(account *Account, payload []byte, message string) bool {
	// Selected-model capacity is request-scoped. A same-account retry only
	// repeats the provider's capacity decision and must not consume the pool's
	// retry budget.
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverErrorWithModel(
	...
	if failoverErr.IsCredentialFailure() || failoverErr.RequestScopedTransient || failoverErr.Reason == openAISelectedModelCapacityReason {
		return failoverErr
	}
