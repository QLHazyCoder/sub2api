...
func TestStreamFailedEventSelectedModelCapacityImmediatelySwitchesAccount(t *testing.T) {
	account := &Account{
		ID:       1,
	...
	payload := []byte(`{"type":"response.failed","response":{"error":{"code":"server_is_overloaded","message":"` + message + `"}}}`)

	require.False(t, openAIStreamFailedEventRetryableOnSameAccount(account, payload, message))
	failoverErr := newOpenAIUpstreamFailoverError(
		http.StatusBadRequest,
		http.Header{"X-Request-Id": []string{"rid-selected-capacity"}},
		payload,
		message,
		true,
	)
