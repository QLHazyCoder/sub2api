...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		writerSizeBeforeForward := service.OpenAISemanticOutputWrittenSize(c)
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.SafeToFailoverAfterWrite && service.OpenAISemanticOutputWrittenSize(c) > 0 {
						streamStarted = true
					}
				...
				if service.ShouldReportOpenAIStreamFailureToScheduler(err) {
					h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, account.GetMappedModel(reqModel), false, nil)
				}
...
func openAIForwardErrorAlreadyCommunicated(c *gin.Context, writerSizeBeforeForward int, err error) bool {
	...
	// 响应已写出（#3887）。
	if service.OpenAISemanticOutputWrittenSize(c) == writerSizeBeforeForward ||
...
func openAIFirstOutputFailoverExhausted(failoverErr *service.UpstreamFailoverError, switchCount *int) bool {
	if failoverErr == nil || !failoverErr.SafeToFailoverAfterWrite ||
		service.IsOpenAIStreamDataIntervalTimeoutFailure(failoverErr) || switchCount == nil {
		return false
	}
